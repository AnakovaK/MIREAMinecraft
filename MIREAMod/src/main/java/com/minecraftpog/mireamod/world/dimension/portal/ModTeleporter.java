package com.minecraftpog.mireamod.world.dimension.portal;

import com.minecraftpog.mireamod.block.ModBlocks;
import com.minecraftpog.mireamod.block.custom.MireaPortalBlock;
import com.minecraftpog.mireamod.villager.ModPOIs;
...
public class ModTeleporter implements ITeleporter {
    ...
    public Optional<BlockUtil.FoundRectangle> getExistingPortal(BlockPos pos) {
        ...
        Optional<PoiRecord> optional = poiManager.getInSquare((poiType) ->
                poiType == ModPOIs.MIREA_PORTAL.get(), pos, 64, PoiManager.Occupancy.ANY).sorted(Comparator.<PoiRecord>comparingDouble((poi) ->
    ...
    public Optional<BlockUtil.FoundRectangle> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        BlockState kaupenPortal = ModBlocks.MIREA_PORTAL.get().defaultBlockState().setValue(MireaPortalBlock.AXIS, axis);
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerLevel level, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        boolean destinationIsUG = level.dimension() == ModDimensions.MIREADIM_KEY;
        if (entity.level.dimension() != ModDimensions.MIREADIM_KEY && !destinationIsUG) {
            return null;
        }
        else {
    ...
    protected Optional<BlockUtil.FoundRectangle> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if(existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.level.getBlockState(entity.portalEntrancePos).getOptionalValue(MireaPortalBlock.AXIS).orElse(Direction.Axis.X);
